...
public:
	...
	const std::vector<DbfField> &GetFields() const {
		return fields;
	}
	uint32_t GetRecordCount() const {
		return header.m_uNumRecords;
	}
	uint16_t GetRecordLength() const {
		return header.m_uRecordSize;
	}
	uint16_t GetHeaderLength() const {
		return header.m_uFirstRecordOffset;
	}
...
private:
	...
	inline void stripUnicode(std::string &str) {
		...
		*/
		str.erase(remove_if(str.begin(), str.end(), [](unsigned char ch) { return !(ch >= 0 && ch < 128); }),
		          str.end());
	}
};
